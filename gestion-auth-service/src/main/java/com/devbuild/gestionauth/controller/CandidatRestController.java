...
public class CandidatRestController {
    ...
    java.util.List<java.util.Map<String,Object>> content = p.getContent().stream().map(d -> {
        ...
        // Expose static URL for public viewing/thumbnails since /uploads/** is now served and public
        if (d.getFilename() != null) {
            m.put("url", "/uploads/" + d.getFilename());
            m.put("previewUrl", "/uploads/" + d.getFilename());
            m.put("thumbnail", "/uploads/" + d.getFilename());
        }
        return m;

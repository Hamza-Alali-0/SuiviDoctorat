...
public class UserService {
    ...
    public boolean requestEmailChange(String currentEmail, String newEmail) {
        if (currentEmail == null || newEmail == null || newEmail.isBlank()) return false;
        try {
            // Check if new email already exists
            if (userRepository.findByEmail(newEmail).isPresent()) {
                try { System.out.println("[UserService] requestEmailChange: new email already exists=" + newEmail); } catch (Throwable t) {}
                return false;
            }
            ...
            // Rate limiting: enforce minimum interval between sends (60 seconds)
            if (u.getVerificationSentAt() != null && u.getVerificationSentAt().isAfter(now.minusSeconds(60))) {
                try { System.out.println("[UserService] requestEmailChange suppressed: sent too recently"); } catch (Throwable t) {}
                return false;
            }
            
            // Generate verification code
            String code = String.format("%06d", new java.util.Random().nextInt(1_000_000));
            u.setPendingEmail(newEmail);
            u.setEmailChangeToken(code);
            u.setEmailChangeExpiry(now.plusMinutes(15));
            u.setVerificationSentAt(now);
            userRepository.save(u);
            ...
            if (notificationUrl != null && !notificationUrl.isBlank()) {
                java.util.Map<String, Object> payload = new java.util.HashMap<>();
                payload.put("to", newEmail); // Send to NEW email
                payload.put("templateCode", "EMAIL_CHANGE_VERIFY");
                java.util.Map<String, String> vars = new java.util.HashMap<>();
                vars.put("code", code);
                vars.put("fullname", (u.getFirstName() != null ? u.getFirstName() : "") + " " + (u.getLastName() != null ? u.getLastName() : ""));
                vars.put("newEmail", newEmail);
                vars.put("currentEmail", currentEmail);
                payload.put("variables", vars);
                try { System.out.println("[UserService] Sending email change verification code to " + newEmail); } catch (Throwable t) {}
                notificationClient.sendEmailRequest(payload)
                    .doOnError(e -> { try { System.err.println("[UserService] requestEmailChange error: " + e.getMessage()); } catch (Throwable t) {} })
                    .subscribe();
            } else {
                try { System.out.println("[UserService] notification service not configured; saved code=" + code + " for new email=" + newEmail); } catch (Throwable t) {}
            }
            return true;
        } catch (Exception ex) {
            try { System.err.println("[UserService] Exception in requestEmailChange: " + ex.getMessage()); ex.printStackTrace(); } catch (Throwable t) {}
            return false;
        }
    }

    public boolean verifyAndChangeEmail(String currentEmail, String code) {
        if (currentEmail == null || code == null) return false;
        try {
            var uOpt = userRepository.findByEmail(currentEmail);
            if (uOpt.isEmpty()) return false;
            User u = uOpt.get();
            
            if (u.getEmailChangeToken() == null || u.getPendingEmail() == null) {
                try { System.out.println("[UserService] verifyAndChangeEmail: no pending email change for user=" + currentEmail); } catch (Throwable t) {}
                return false;
            }
            
            String storedCode = u.getEmailChangeToken().trim();
            String inputCode = code.trim();
            
            if (!storedCode.equals(inputCode)) {
                try { System.out.println("[UserService] verifyAndChangeEmail: code mismatch"); } catch (Throwable t) {}
                return false;
            }
            
            java.time.LocalDateTime now = java.time.ZonedDateTime.now(java.time.ZoneOffset.UTC).toLocalDateTime();
            if (u.getEmailChangeExpiry() != null && u.getEmailChangeExpiry().isBefore(now)) {
                try { System.out.println("[UserService] verifyAndChangeEmail: code expired"); } catch (Throwable t) {}
                return false;
            }
            
            // Check again if new email is still available
            if (userRepository.findByEmail(u.getPendingEmail()).isPresent()) {
                try { System.out.println("[UserService] verifyAndChangeEmail: new email now taken=" + u.getPendingEmail()); } catch (Throwable t) {}
                u.setPendingEmail(null);
                u.setEmailChangeToken(null);
                u.setEmailChangeExpiry(null);
                userRepository.save(u);
                return false;
            }
            
            try { System.out.println("[UserService] verifyAndChangeEmail: SUCCESS - changing email from " + u.getEmail() + " to " + u.getPendingEmail()); } catch (Throwable t) {}
            ...
            try { System.err.println("[UserService] Exception in verifyAndChangeEmail: " + ex.getMessage()); ex.printStackTrace(); } catch (Throwable t) {}
            return false;
        }
    }
    ...
    public User updateFullProfile(String email, java.util.Map<String, Object> updates) {
        User u = userRepository.findByEmail(email).orElseThrow();
        boolean changed = false;
        if (updates.containsKey("firstName")) { u.setFirstName((String) updates.get("firstName")); changed = true; }
        if (updates.containsKey("lastName")) { u.setLastName((String) updates.get("lastName")); changed = true; }
        if (updates.containsKey("phone")) { u.setPhone((String) updates.get("phone")); changed = true; }
        if (updates.containsKey("affiliation")) { u.setAffiliation((String) updates.get("affiliation")); changed = true; }
        if (updates.containsKey("bio")) { u.setBio((String) updates.get("bio")); changed = true; }
        if (updates.containsKey("linkedinUrl")) { u.setLinkedinUrl((String) updates.get("linkedinUrl")); changed = true; }
        if (updates.containsKey("portfolioUrl")) { u.setPortfolioUrl((String) updates.get("portfolioUrl")); changed = true; }
        if (changed) return userRepository.save(u);
        return u;
    }

    public User saveAvatar(String email, String avatarUrl) {
        User u = userRepository.findByEmail(email).orElseThrow();
        u.setAvatar(avatarUrl);
        return userRepository.save(u);
    }

    public User saveCv(String email, String cvUrl) {
        User u = userRepository.findByEmail(email).orElseThrow();
        u.setCvUrl(cvUrl);
        return userRepository.save(u);
    }

...
@Component({
  ...
  template: `
    <nav class="navbar">
      <div class="nav-left">
        <div class="brand">
          <a routerLink="/" class="brand-text">{{ tx.t('navbar.brand') }}</a>
        </div>
      </div>
      ...
      <div class="nav-right">
        ...
        <button
          ...
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
            ...
            <path d="M2 12h20M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1 4-10z" stroke="currentColor" stroke-width="2"/>
          </svg>
          <span style="font-size: 0.75rem; font-weight: 700; margin-left: 4px;">{{ tx.current().toUpperCase() }}</span>
        </button>
        ...
        <div class="profile-dropdown" *ngIf="showProfileMenu" (click)="closeProfileMenu()">
          ...
          <div class="dropdown-content" (click)="$event.stopPropagation()">
            <a routerLink="/profile-selection" class="dropdown-item" (click)="closeProfileMenu()">{{ tx.t('navbar.profile.select') }}</a>
            <div class="dropdown-divider"></div>
            <button class="dropdown-item" (click)="logout()">{{ tx.t('navbar.profile.logout') }}</button>
          </div>
        </div>
      </div>
    </nav>
  ...
  styles: [`
    ...
    .icon-button{ border:0; background:transparent; width:auto; height:36px; padding: 0 8px; border-radius:8px; display:inline-flex; align-items:center; justify-content:center; cursor:pointer; color:var(--color-text) }
    ...
    .profile-initials{ width:32px; height:32px; border-radius:999px; background:#e6eef8; color:#1e40af; display:flex; align-items:center; justify-content:center; font-weight:700 }
    ...
    .dropdown-item{ display:block; padding:0.6rem; color:var(--color-text); text-align:left; width:100%; background:transparent; border:0; cursor:pointer; text-decoration:none; font-size: 0.9375rem; }
...
export class UserNavbarComponent implements OnInit {
  ...
  constructor(private auth: AuthService, private router: Router, public tx: TranslationService) {}
  ...
  changeLanguage() {
    const current = this.tx.current();
    const next = current === 'en' ? 'fr' : current === 'fr' ? 'ar' : 'en';
    this.tx.setLanguage(next);
  }

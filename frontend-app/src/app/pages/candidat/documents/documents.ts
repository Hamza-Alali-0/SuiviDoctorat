...
export class DocumentsPage {
  ...
  downloadDocument(id: number|string){
    // Prefer using a public/static URL if the backend exposed one (same as profile CV link)
    const doc = this.docs().find(d => d.id === id);
    if (doc && doc.url) {
      try {
        const a = document.createElement('a');
        a.href = doc.url;
        const filename = (doc.originalFilename || doc.original_filename || doc.filename || doc.fileName || (doc.url || '').split('/').pop() || `document-${id}`);
        a.download = filename;
        ...
        console.warn('Direct download via public URL failed, falling back to API', e);
      }
    }
    ...
    this.http.get(url, { responseType: 'blob' }).subscribe({
      next: (blob) => {
        ...
        const a = document.createElement('a');
        a.href = dlUrl;
        const filename = (doc && (doc.originalFilename || doc.original_filename || doc.filename || doc.fileName)) || `document-${id}`;
        a.download = filename;
        a.click();
  ...
  async openDocument(id: number|string){
    // Check if we have a static URL for this document (public access)
    const doc = this.docs().find(d => d.id === id);
    if (doc && doc.url) {

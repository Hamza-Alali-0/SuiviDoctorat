...
export class ApplicationsPage implements OnInit {
  ...
  constructor(
    ...
    this.applicationsService.getApplicationsUpdated().subscribe(() => {
      console.log('[ApplicationsPage] ✓ Detected application update notification, refreshing in 1s...');
      // Add a small delay to allow backend to process the transaction and update the view
      setTimeout(() => {
  ...
  fetchApplications(): void {
    ...
    this.authService.getProfile().subscribe({
      next: (user: any) => {
        ...
        if (!userId) {
          console.warn('[ApplicationsPage] Profile loaded but no ID found, attempting token parse...');
          userId = this.extractIdFromToken();
        }

        // If still no userId, try calling getMyApplications without ID (uses /me endpoint)
        if (!userId) {
          console.log('[ApplicationsPage] No numeric user id available. Attempting to fetch using /me endpoint...');
          this.applicationsService.getMyApplications().subscribe({
            next: (data) => this.handleApplicationsData(data),
            error: (err) => {
              console.error('[ApplicationsPage] ✗ Failed to fetch applications (me):', err);
              this.loading = false;
            }
          });
          return;
  ...
  handleApplicationsData(data: any): void {
  ...
  extractIdFromToken(): number | null {
    try {
      ...
      if (token) {
        ...
        if (parts.length >= 2) {
          ...
          if (cand && !isNaN(Number(cand))) {
            ...
            console.log('[ApplicationsPage] Recovered userId from token:', id);
            return id;
          }
        }
      }
    } catch (e) { console.error('[ApplicationsPage] Token parse failed:', e); }
    return null;
  }
}

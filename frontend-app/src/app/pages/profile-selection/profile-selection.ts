    ...
    try {
      this.auth.getMyRoleRequests().subscribe({ next: (list:any[]) => {
        ...
        if (pending){
          ...
          // Update selected role based on pending request
          if (pending.requestedRole) {
            const roleMap: Record<string, string> = {
              'ROLE_ENCADRANT': 'directeur',
              'ROLE_ADMIN': 'admin',
              'ROLE_CANDIDAT': 'candidat'
            };
            const mappedRole = roleMap[pending.requestedRole] || pending.requestedRole.toLowerCase().replace('role_', '');
            this.selected.set(mappedRole);
          }
  ...
  startPollingForApproval(){
    ...
    const poll = () => {
      try { this.auth.getMyRoleRequests().subscribe({ next: (list:any[]) => {
        ...
        // Update pending request to show latest admin message if any
        if (pending && this.pendingRequest) {
          this.pendingRequest = pending;
        }
        
        if (approved){
          // approved: refresh user profile to get updated role, then navigate to dashboard
          this.awaiting.set(false);
          if (this._countdownTimer) clearInterval(this._countdownTimer);
          if (this._pollTimer) clearInterval(this._pollTimer);
          console.log('[ProfileSelection] Request approved, refreshing user profile...');
          // Reload user profile to update the role in auth service
          this.auth.getProfile().subscribe({
            next: (profile: any) => {
              console.log('[ProfileSelection] Profile refreshed:', profile);
              const newRole = profile?.role || this.auth.role?.();
              // Update auth service with new role
              if (newRole) {
                const token = this.auth.getToken();
                this.auth.setAuth(token, newRole);
              }
              // Navigate to appropriate dashboard based on new role
              if (newRole === 'ROLE_CANDIDAT' || newRole?.toLowerCase().includes('candidat')) {
                console.log('[ProfileSelection] Navigating to candidat dashboard');
                this.router.navigate(['/candidat/dashboard']);
              } else if (newRole === 'ROLE_ENCADRANT' || newRole?.toLowerCase().includes('encadrant')) {
                console.log('[ProfileSelection] Navigating to encadrant dashboard');
                this.router.navigate(['/encadrant/dashboard']);
              } else if (newRole === 'ROLE_ADMIN' || newRole?.toLowerCase().includes('admin')) {
                console.log('[ProfileSelection] Navigating to admin dashboard');
                this.router.navigate(['/admin']);
              } else {
                // Fallback: reload the entire page to ensure state is fresh
                console.log('[ProfileSelection] Unknown role, reloading page');
                window.location.reload();
              }
            },
            error: (err) => {
              console.error('[ProfileSelection] Error refreshing profile:', err);
